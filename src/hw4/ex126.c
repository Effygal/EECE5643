...
#define FILENAME   "../c_lib/ac.dat"                  /* input data file */
#define START      0.0
//CODE
#define JOBNUM    500
...
//CODE
  ...
  fptr = fopen("ac_services.txt", "w");
  
  // Print each element of the array to the file
  int len = sizeof(services) / sizeof(services[0]);
  for(int i = 0; i < len; i++) {
      fprintf(fptr, "%6.2f\n", services[i]);
  }
  // Close the file
